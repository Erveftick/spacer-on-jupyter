...
def main():
    ...
    fp = setup_fixedpoint()
    queries = parse_queries(fp, code)
    fp_rules = extract_rules(fp)
    magic_values = find_magic_values(fp_rules)
    magic_values_vars, substitutions = prepare_substitution(magic_values)
    subs_rules = apply_substitution(fp_rules, substitutions)
    processed_rules = process_first_rule(subs_rules, substitutions)

    new_rules = create_new_rules(processed_rules, magic_values_vars)
    new_vars = create_new_vars(processed_rules)
